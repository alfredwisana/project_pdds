...
$query = "
    ...
    FROM
        customers c
    LEFT JOIN
        orders ol ON c.CustomerID = ol.CustomerID
    GROUP BY
...
if (isset($_GET['country']) && $_GET['country'] === 'All') {
    $query .= "GROUP BY c.CustomerID, c.CompanyName, c.Country ORDER BY TotalSpending DESC;";
    $stmts = $conn->prepare($query);
} elseif (isset($_GET['country']) && $_GET['country'] !== '') {
    $query .= "WHERE c.Country = :country GROUP BY c.CustomerID, c.CompanyName, c.Country ORDER BY TotalSpending DESC;";
    $stmts = $conn->prepare($query);
    $stmts->bindParam(':country', $_GET['country']);
}else{
    $query .= "GROUP BY c.CustomerID, c.CompanyName, c.Country ORDER BY TotalSpending DESC;";
    $stmts = $conn->prepare($query);   
}
    ...
    <div class="container mt-4 w-1/2 d-flex justify-center">
        <div class="card mb-4 shadow-xl p-2">
            <div class="card-body text-center">
                ...
                //    $grand_total_query = "SELECT SUM(TotalPrice) AS GrandTotal FROM orders";

                if ($countryFilter == 'All') {
                    $grand_total_query = "SELECT SUM(TotalPrice) AS GrandTotal FROM orders";
                } else {
                    $grand_total_query = " SELECT SUM(TotalPrice) AS GrandTotal FROM orders WHERE CustomerID IN (SELECT CustomerID FROM customers WHERE Country = :country)";
                }
